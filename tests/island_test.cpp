...
TEST_CASE("content slides use deterministic horizontal directions") {
  ...
  left.update(0.1F);
  CHECK(left.visual().outgoing_opacity == Approx(1.0F));
  CHECK(left.visual().incoming_opacity == Approx(1.0F));
  CHECK(left.visual().outgoing_offset_x == Approx(-24.0F));
  CHECK(left.visual().incoming_offset_x == Approx(24.0F));
